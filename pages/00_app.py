...
- **빨간색**: 독거노인 인구 대비 의료기관이 **가장 부족한 지역** (설정된 기준값보다 낮음)
- **초록색**: 독거노인 인구 대비 의료기관이 **상대적으로 많은 지역** (설정된 기준값보다 높음)
""")
...
if df_elder is not None and df_facility is not None:
    ...
    if target_col is not None and target_col in df_elder.columns:
        ...
        # -----------------------------
        # 지도 시각화를 위한 10번째 백분위수 계산 (***최종 수정 부분***)
        # -----------------------------
        # 10번째 백분위수 (하위 10%)를 색상 중앙값으로 설정
        percentile_10 = df["독거노인_1000명당_의료기관_수"].quantile(0.10)
        ...
        fig = px.choropleth(
            ...
            # ⭐ 색상 척도의 중앙값(노란색)을 10번째 백분위수로 설정
            color_continuous_scale="RdYlGn", 
            color_continuous_midpoint=percentile_10, 
            title=f"시도별 독거노인 **1000명당** 의료기관 분포 (기준값: {percentile_10:.2f})", 
