...
st.markdown("""
**독거노인 인구수 대비 의료기관 분포를 시도별로 시각화합니다.**
...
if df_elder is not None and df_facility is not None:
    ...
    # 🔠 지역 컬럼 탐색
    # -----------------------------
    elder_region_col = [c for c in df_elder.columns if any(k in c for k in ["시도", "행정구역", "지역"])]
    ...
    df_elder["지역"] = df_elder[elder_region].apply(normalize_region)
    df_facility["지역"] = df_facility[facility_region].apply(normalize_region)
    ...
    # 이름 확인 (디버그용)
    # st.write(sorted(df["지역"].unique()))
    # st.write([f['properties']['name'] for f in geojson['features']])
    ...
    fig = px.choropleth(
        ...
        title="시도별 독거노인 인구 대비 의료기관 분포 (의료기관 수 ÷ 독거노인 인구수)",
